...
import { describeCall, parseCapture } from '../lib/sipLadder.js'

function MetricCard({ label, value, rating, hint }) {
...
function formatSec(sec, digits = 1) {
  if (sec == null || Number.isNaN(sec)) return '—'
  return `${sec.toFixed(digits)}s`
}

function CallPicker({ calls, onSelect }) {
  return (
...
function Findings({ findings, hasError }) {
  if (!findings?.length) {
    return (
    ...
    <div className="cd-ladder-panel">
      ...
      <div className="cd-ladder" role="table" aria-label="SIP message ladder">
        <div className="cd-ladder-cols" role="row">
          ...
          <div className="cd-ladder-col-head">NetSapiens</div>
              ...
              >
                ...
                >
                  ...
                  <span className="cd-ladder-mobile-dir">
                    {ev.fromLane === 'phone' ? call.phoneLabel : ev.fromLane === 'far' ? call.farEndLabel : 'NetSapiens'}
                    {' → '}
                    {ev.toLane === 'phone' ? call.phoneLabel : ev.toLane === 'far' ? call.farEndLabel : 'NetSapiens'}
                  </span>
                  <span
...
function AnalysisView({ call, onBack, onPickOther, multi }) {
